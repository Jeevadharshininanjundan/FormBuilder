...
// Change this file: /routes/forms.js
...
const validateFormData = (form) => {
  ...
  form.questions.forEach((question, index) => {
    ...
    if (question.type === 'categorize') {
      ...
      if (!question.clozeText || !question.clozeText.includes('<u>')) {
        errors.push(`Question ${index + 1} (Cloze) must have underlined blanks (use the 'U' button).`);
      }
      if (!question.options || question.options.length === 0) {
        errors.push(`Question ${index + 1} (Cloze) must have at least one blank option.`);
      }
    } 
...
// The rest of your router.post, router.get, router.put remain unchanged.
// Your server.js is also fine as it is.
