...
import QuestionEditor from '../components/QuestionEditor'; // Ensure this path is correct
...
export default function Builder() {
    ...
    // New state to manage the current step of the form builder
    const [step, setStep] = useState(1); // 1: Form Details, 2: Questions, 3: Review/Save
    ...
    function addQuestion(type) {
        ...
        setQuestions((prev) => [...prev, newQuestion]);
        setSelectedIndex(questions.length); // Select the newly added question
    }
    ...
    function removeQuestion(idx) {
        ...
        if (selectedIndex === idx) {
            setSelectedIndex(null); // Deselect if the removed question was selected
        } else if (selectedIndex > idx) {
            setSelectedIndex((prevIdx) => prevIdx - 1); // Shift index if needed
        }
        // Also remove its upload status
        setQuestionUploadStatuses(prev => {
            const newStatuses = { ...prev };
            delete newStatuses[idx];
            // Adjust subsequent indices if needed (optional, but good for cleanup)
            const adjustedStatuses = {};
            Object.keys(newStatuses).forEach(key => {
                const numKey = parseInt(key);
                if (numKey > idx) {
                    adjustedStatuses[numKey - 1] = newStatuses[key];
                } else {
                    adjustedStatuses[numKey] = newStatuses[key];
                }
            });
            return adjustedStatuses;
        });
    }
    ...
    async function saveForm() {
        ...
        try {
            ...
            // Do NOT set setSubmissionStatus('error') here, as alerts handle feedback.
        }
    }
    ...
    const goToNextStep = () => {
        ...
        if (step === 1) {
            if (!title.trim()) {
                alert('Please enter a form title before proceeding.');
                return;
            }
            // Optionally, add validation for header image if it's mandatory
        }
        ...
        if (step === 2) {
            ...
            // You might want more detailed validation for questions here before saving
            // For example, ensuring all required fields within each question type are filled.
            // This is partially handled by backend, but frontend validation is good UX.
        }
    ...
    return (
        <div className="max-w-full min-h-screen bg-[#D4F1F4] pt-[100px] p-8 flex mx-auto gap-8">
            {/* Left Panel: Question List (Only visible in Step 2) */}
            {step === 2 && (
                <div className="w-60 bg-[#FFE8D6] rounded-xl p-4 flex flex-col gap-4 overflow-auto max-h-[calc(100vh-120px)] border border-[#FF6F91]/50 shadow-lg">
                    {questions.length === 0 && (
                        <p className="text-[#116466]/70 text-lg font-light italic px-4">
            ...
            {/* Center Panel: Form Editor Content based on Step */}
            <div className="flex-1 flex flex-col gap-6">
                {/* Dynamically change header based on step */}
                {step === 3 ? (
                    <h1 className="text-[#116466] font-bold text-3xl">Review Your Form</h1>
                ) : (
                    <h1 className="text-[#116466] font-bold text-3xl">Create a Form</h1>
                )}

                {step === 1 && ( // Step 1: Form Title & Header Image
                    <>
                        <div>
                            <label className="block mb-2 font-bold text-[#116466]">Form Title</label>
                            <input
                ...
                {step === 2 && ( // Step 2: Question Editor
                    selectedIndex !== null && questions[selectedIndex] ? (
                        ...
                        <div className="mt-8 p-4 bg-gray-100 border border-gray-300 text-gray-700 rounded-lg shadow-md text-center">
                            <p className="font-semibold text-lg">Select a question from the left panel to edit, or add a new one from the right.</p>
                        </div>
                    )
                )}

                {step === 3 && ( // Step 3: Review / Save Confirmation
                    submissionStatus === 'success' ? (
                        <div className="mt-8 p-4 bg-green-100 border border-green-400 text-green-700 rounded-lg shadow-md text-center">
                            <p className="font-semibold text-lg">✅ Form created successfully!</p>
                            <p className="mt-2 text-md">
                        ...
                        <div className="mt-8 p-6 bg-white border border-[#116466]/30 text-[#333] rounded-xl shadow-lg"> {/* Enhanced UI */}
                            <h2 className="text-2xl font-bold text-[#FF6F91] mb-4 text-center">Form Summary</h2> {/* Enhanced UI */}
                            <p className="mb-6 text-center text-lg text-[#555]">
                                Please review your form details before saving.
                            </p>
                            <div className="space-y-3 text-left p-4 bg-[#f0f9f9] rounded-lg border border-[#FFB6B3]"> {/* Enhanced UI */}
                                <p className="text-lg"><span className="font-semibold text-[#116466]">Form Title:</span> {title || '<No Title Provided>'}</p> {/* Enhanced UI */}
                                <p className="text-lg"><span className="font-semibold text-[#116466]">Header Image:</span> {headerImage ? 'Yes (Image Attached)' : 'No (No Image)'}</p> {/* Enhanced UI */}
                                <p className="text-lg"><span className="font-semibold text-[#116466]">Number of Questions:</span> {questions.length}</p> {/* Enhanced UI */}
                                {questions.length > 0 && (
                                    <div className="mt-3">
                                        <p className="font-semibold text-[#116466]">Questions Added:</p> {/* Enhanced UI */}
                                        <ul className="list-disc list-inside ml-6 mt-2 text-base"> {/* Enhanced UI */}
                                            {questions.map((q, qIdx) => (
                                                <li key={qIdx}>{`Q${qIdx + 1}: ${q.type.charAt(0).toUpperCase() + q.type.slice(1)}`} {q.questionText && ` - "${q.questionText.substring(0, 30)}..."`}</li> 
                                            ))}
                                        </ul>
                                    </div>
                                )}
                ...
                <div className="mt-10 flex justify-between">
                    {/* Previous button for Step 2 and Step 3 */}
                    {step > 1 && submissionStatus !== 'success' && (
                        <button
                            onClick={goToPreviousStep}
                            className="bg-[#FFB6B3] hover:bg-[#ff9e9b] transition px-6 py-3 rounded-lg text-white text-lg font-bold shadow-lg"
                        >
                            Previous
                        </button>
                    )}
                    {/* Next button (Step 1 -> Step 2, Step 2 -> Step 3) */}
                    {step < 3 && submissionStatus !== 'success' && (
                        <button
                            onClick={goToNextStep}
                            className="bg-[#116466] hover:bg-[#0e4f48] transition px-6 py-3 rounded-lg text-white text-lg font-bold shadow-lg ml-auto"
                        >
                            Next
                        </button>
                    )}
                    {/* Save Form button (Only in Step 3, if not already submitted) */}
                    {step === 3 && submissionStatus !== 'success' && (
                        <button
                            onClick={saveForm}
                            className="bg-[#FF6F91] hover:bg-[#d85873] transition px-8 py-3 rounded-lg text-white text-xl font-bold shadow-lg ml-auto"
                        >
                            Save Form
                        </button>
                    )}
            ...
            {/* Right Panel: Question Type Selector (Only visible in Step 2) */}
            {step === 2 && (
                <aside className="w-72 bg-[#FFE8D6] rounded-xl p-6 flex flex-col gap-6 shadow-lg border border-[#FF6F91]/50">
                    <h2 className="text-[#FF6F91] font-extrabold text-xl tracking-wide uppercase">Question Type</h2>
                    {QUESTION_TYPES.map(({ type, title, description }) => (
