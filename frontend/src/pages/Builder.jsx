...
export default function Builder() {
  ...
  const [headerUploadStatus, setHeaderUploadStatus] = useState(null); // Separate state for header
  const [questions, setQuestions] = useState([]);
  const [selectedIndex, setSelectedIndex] = useState(null);
  const [submissionStatus, setSubmissionStatus] = useState(null);
  const [formLink, setFormLink] = useState('');
  
  // New state to manage upload status for each question individually
  ...
  const handleUploadHeader = async (file) => {
    ...
    try {
      ...
      setHeaderImage(null); // Clear image on error
    } finally {
      if (previewUrl) {
  ...
  // New function to handle question image uploads
  const handleUploadQuestionImage = async (qIdx, file) => {
    const previewUrl = URL.createObjectURL(file);

    // Update the upload status for the specific question
    setQuestionUploadStatuses(prev => ({ ...prev, [qIdx]: 'loading' }));

    // Immediately update the question with a local preview URL
    const newQuestions = [...questions];
    newQuestions[qIdx].imageUrl = previewUrl;
    setQuestions(newQuestions);

    try {
      // Endpoint is '/api/upload-image' for both header and question images
      const uploadedUrl = await uploadImageToBackend(file, '/api/upload-image');

      // Update the question with the final backend URL
      ...
      // On error, clear the image and set status
      const updatedQuestions = [...questions];
        ...
        {selectedIndex !== null && questions[selectedIndex] && (
          <div className="border border-[#116466] rounded-lg p-6 bg-white shadow-md mt-4">
            ...
            <QuestionEditor
              ...
              onUploadImage={(file) => handleUploadQuestionImage(selectedIndex, file)} // Pass the new handler
              uploadStatus={questionUploadStatuses[selectedIndex]} // Pass the specific upload status
            />
          </div>
        )}
